...
RUN /arrow/ci/scripts/install_vcpkg.sh /usr/local/vcpkg
...
#RUN export VCPKG_ROOT=/usr/local/vcpkg
#RUN /usr/local/vcpkg/vcpkg install --x-install-root=${VCPKG_ROOT}/installed --x-manifest-root=/arrow/ci/vcpkg --x-feature=flight --x-feature=gcs --x-feature=json --x-feature=parquet --x-feature=gandiva --x-feature=dev
# Build JNI libs
#RUN export VCPKG_DEFAULT_TRIPLET=arm64-linux
#RUN /arrow/ci/scripts/java_jni_manylinux_build.sh /arrow /arrow/build /arrow/dist
